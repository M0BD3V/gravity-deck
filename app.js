...
function applyScanResult(games, folderLabel, scanConfig = {}, apps = currentApps) {
  ...
  currentApps = filterAppsForLibrary(Array.isArray(apps) ? apps : [], currentGames);
...
function createGridToolbar() {
  ...
  toolbarActions.append(createSortSelect(), sizeControls);
...
function createBottomNav() {
  ...
  if (activeLibrary === "catalog") {
    tools.append(createRescanButton(), createSortButton(), createMobileButton(), createHelpButton());
    nav.append(tools);
    return nav;
  }
...
function createMobileButton() {
  const button = createElement("button", "nav-icon nav-action mobile-qr-nav", "QR");

  button.type = "button";
  button.title = "Mobile / Wake-on-LAN";
  button.setAttribute("aria-label", button.title);
  button.addEventListener("click", async (event) => {
    event.stopPropagation();
    playUiSound("toggle");
...
function createDetailsActions(game) {
  ...
  actions.append(play, cover, hide, close);
...
async function launchGame(game, button) {
  if (!game?.exe && !game?.launchUri && !game?.steamAppId) {
  ...
  try {
    await window.mobdeck.launchGame({
      ...game,
      minimizeOnLaunch
    });
    markGameLaunched(game);
...
function restoreSavedLibrary() {
  ...
  currentGames = savedGames;
  currentApps = filterAppsForLibrary(savedApps, currentGames);
